...
if(!elgg_is_active_plugin("amap_maps_api")){
    register_error(elgg_echo("photosmap:settings:amap_maps_api:notenabled"));
    forward(REFERER);
}
...
$title = elgg_echo('photosmap:title');
$options = array(
    "type" => "object",
    "subtype" => "image",
...
if ($initial_load) {
    if ($initial_load == 'newest') {
        ...
        $title = elgg_echo('photosmap:newest', array($options['limit']));
        ...
        if ($user->location) {
            ...
            if ($s_lat && $s_long) {
                $s_radius = amap_ma_get_initial_radius('photosmap');
                ...
                $title = elgg_echo('photosmap:nearby:search', array($user->location));
            }
        }
    }
} else {
    ...
    if ($coords) {
        ...
        $title = elgg_echo('photosmap:nearby:search', array($search_location_txt));
    }
}
